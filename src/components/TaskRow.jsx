...
const TaskRow = React.memo(({ task, onToggle, onUpdateNote, onUpdateLinks }) => {
  ...
  return (
    <>
      <div className={`group relative flex flex-col p-4 md:p-6 my-3 rounded-2xl border transition-all duration-300 overflow-hidden
          ${task.isCompleted 
      ...
      >
        ...
        <div onClick={handleRowClick} className="flex items-start gap-3 md:gap-5 cursor-pointer relative z-10">
          ...
          <div className="flex flex-col flex-1 transition-all duration-300 min-w-0">
            ...
            {/* TEXT CONTAINER */}
            <div className={`transition-all duration-300 ${task.isCompleted ? 'opacity-50' : 'opacity-100'}`}>
              ...
              <p className="text-slate-600 dark:text-slate-400 text-sm md:text-base leading-relaxed pr-2 md:pr-4 mb-4">
                {task.directive}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-1">
              <button 
                ...
                className={`font-bold rounded-lg transition-all flex items-center gap-2 active:scale-95
                  ${task.isCompleted
              ...
              {/* Persistent Link Pills */}
              {!isExpanded && links.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {links.map((link, idx) => (
                    <span key={idx} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-white border border-slate-200 text-slate-600 shadow-sm dark:bg-slate-900/50 dark:border-slate-700 dark:text-slate-300">
        ...
        {/* FULL WIDTH INLINE STUDY SPACE */}
        {/* FIX: Removed pl-11/pl-14, this now spans edge-to-edge inside the card */}
        {isExpanded && !task.isCompleted && (
          <div className="mt-5 relative z-10 border-t border-slate-100 dark:border-slate-800 pt-5">
            <div className="mb-4 animate-fade-in-up" onClick={e => e.stopPropagation()}>
              <form onSubmit={handleAddLink} className="flex gap-2 mb-3">
                <input 
                  ...
                  /* FIX: text-base (16px) added to prevent iOS auto-zoom on tap */
              ...
              {links.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {links.map((link, idx) => (
                    <div key={idx} className="group/pill inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:border-blue-400">
                      <a href={link} target="_blank" rel="noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1.5 truncate max-w-[150px] md:max-w-xs">
